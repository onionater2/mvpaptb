function [subj, outputpatname] = feature_select_SL(subj,data_patin,regsname,selsgroup,numfolds, fs_args, varargin)
...
args.new_maskstem = [args.new_maskstem '_top' num2str(fs_args.fixednum)];
end
...
for n=1:nIterations
  ...
  %figure out the initialfs map for this iteration. if it's a cell, you
  %want to take the one for this iteration
  if iscell(fs_args.initialfs_pat_map)
     fs_args.curiterationfs_pat_map=fs_args.initialfs_pat_map{n};
  else
     fs_args.curiterationfs_pat_map=fs_args.initialfs_pat_map; 
  end
  ...
  subj = statmap_fh(subj,cur_data_patname,regsname,cur_selname,cur_map_patname,fs_args, args.statmap_arg);
...
outputpatname=args.new_map_patname;
