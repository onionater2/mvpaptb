function prep_for_mvpaptb_EIB_main(study, task, inputsubjectlist, resultfolder, runsrange)
...
roidirectory='autoROI_new' % or 'autoROI'
...
subject=inputsubjectlist{s}
    ...
    try
        ...
        warning 'subject has fewer runs than specified in runsrange'
    end
...
for roin=1:8
    roi_descriptions_ind{roin}=[ROISind{roin} '_ind'];
end
for roin=9:length(ROISind)
    roi_descriptions_ind{roin}=[ROISind{roin}(5:end) '_ind'];
end
...
%save the relevant structures for the subject
 save([mvpadir 'subjinfo_' task '_' runsincluded], 'boldnames', 'maskname', 'mvpa_mask_description', 'allconds_binarized', 'allconds_convolved',  'allcond_names','blocklabels', 'blocklabelscomb', 'runselectors', 'evenoddselectors', 'artselectors', 'restselectors', 'combselectors', 'stimselector', 'crossmatchedruns', 'contextselector', 'genderselector', 'crossrunsONEselector', 'crossrunsTWOselector','foldlabels', 'TR', 'subjvoxelcount', 'voxelcountlabel', 'wart');
 save([mvpadir 'discriminations_' task '_' runsincluded], 'alleight', 'faceVScontext', 'negVSpos', 'negVSposONE', 'negVSposTWO', 'negfVSposf', 'negcVSposc', 'maleVSfemale', 'socialVSnonsoc','socialnVSsocialp', 'nonsocnVSnonsocp', 'malenVSmalep', 'femalenVSfemalep');
    end
...
if includeStimLabels & sum(ismember(stims.subjectlist, subject))
    save([[mvpadir 'trialinfo_' task '_' runsincluded]], 'stimlabels_nums', 'stimlabels_names', 'stimresponses', 'context_stimlabels_nums', 'context_stimresponses', 'persons_stimlabels_nums', 'persons_stimresponses')
    ...
    save(['/mindhive/saxelab2/' study '/mvpaptb/' task '_voxelcountsummary.mat'], 'voxelcount', 'voxelcountlabel'); %save a general summary of the rois across subjects
end
