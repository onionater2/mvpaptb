function run_classification_EIB(study, task, subjectlist, runsincluded, varargin)
...
folderprefix='final';
masksubset={'lSTS_peelenpeak_xyz_group', 'rSTS_peelenflip_xyz_group'};%'MMPFC_tomloc_ind', 'DMPFC_tomloc_ind', 'VMPFC_tomloc_ind'};%'rSTS_kanparcel_EmoBioLoc_ind','rFFA_kanparcel_EmoBioLoc_ind','rOFA_kanparcel_EmoBioLoc_ind', 'rpSTS_BDbiomot_ind', 'lSTS_peelenpeak_xyz_group','rSTS_peelenflip_xyz_group'};%&'lSTS_peelenpeak_xyz_group', 'MPFC_peelenpeak_xyz_group', 'rSTS_peelenflip_xyz_group'};%'lSTS_kanparcel_EmoBioLoc_ind', 'rSTS_kanparcel_EmoBioLoc_ind','lOFA_kanparcel_EmoBioLoc_ind','rOFA_kanparcel_EmoBioLoc_ind','lFFA_kanparcel_EmoBioLoc_ind','rFFA_kanparcel_EmoBioLoc_ind'}; % 'whole_brain_grey_matter'%{'VMPFC_tomloc_ind'} %based on names in subjinfo mvpa_mask_description
discsubset={}; %, 'negVSposONE', 'negVSposTWO'}; %based on names in discrimination .mat file
studydir=[rootdir study '/'];
...
killmats=1; %default 1 will delete the actual patterns.
checkbeforedelete=0;
...
other_args.fixed=1; %this specifies whether you are using rank or threshold based feature selection
...
if other_args.cross_subj
  other_args.featureselect=0; %automatically sets featureselect to 0 if doing cross subject decoding  
end
other_args.notes='unique information analysis'; % write yourself some handy notes about this analysis
