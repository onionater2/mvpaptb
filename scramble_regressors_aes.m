function [subj] = scramble_regressors(subj,regsname,selname,new_regsname,iteration4seed, varargin)
...
s = RandStream.create('mt19937ar','seed',sum(100*iteration4seed*clock));
for i = 1:nruns
  thisrun = find(runs == i);
  %regs(:,thisrun) = shuffle(regs(:,thisrun),2); %shuffle sucks
  randindices=randperm(s, length(thisrun));
