function [subj] = statmap_searchlight_AES(subj,data_patname,regsname,selname,new_map_patname,fs_args, extra_arg)
...
xval2_idx = find(sel==2);
%  create the pat2 and regs2 matrices, e.g. for use in
% statmap_classify.m.
if length(xval2_idx)

  % the actual subset of pat2 that gets sent in will be
...
% using)
%for v = 1:nVox
for v = fs_args.voxelrange
% %AES edit to allow for computing partial maps and later taking their
% union (ghetto way to parallelize this process)
...
if fs_args.fixednum<length(sortIndex)
      vox_idx=vox_idx(sortIndex(1:fs_args.fixednum));
  else
      vox_idx=vox_idx; % less than fixednum, so just take 'em all!
end
%now limit the sphere to those voxels
sphere=pat1(vox_idx,:);
else if fs_args.fstype==2
        error('sorry, I have not implemented threshold-based feature selection within searchlight spheres. but cmon, it would be such a pain in the ass.')
    end
end
  
  
  scratch.pat2 = pat2(vox_idx,:);
  ...
  [hidethecraziness, map(v) scratch] = evalc('funct_h(sphere, regs1, scratch)');
  %[hidethecraziness, map(v) scratch(v)] = evalc('funct_h(sphere, regs1,scratch(v))'); % AES edit to make nonsequential
  %[hidethecraziness, map(v)] = evalc('funct_h(sphere, regs1, scratch)');
end
